...
static struct synaptics_i2c_rmi_platform_data mecha_ts_synaptics_data[] = { /* Synatpics sensor */
	{
		.version = 0x3330,
...
{
	printk(KERN_INFO "%s():\n", __func__);
	gpio_direction_output(PM8058_GPIO_PM_TO_SYS(MECHA_TP_RSTz), 0);
	mdelay(10);
	gpio_direction_output(PM8058_GPIO_PM_TO_SYS(MECHA_TP_RSTz), 1);
}
...
static ssize_t mecha_himax_virtual_keys_show(struct kobject *kobj,
...
static ssize_t mecha_himax_virtual_keys_show_china_mfg(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
...
static ssize_t mecha_himax_virtual_keys_show_china(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)	":65:849:80:50"
...
static ssize_t mecha_atmel_virtual_keys_show(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
...
static ssize_t mecha_synaptics_virtual_keys_show(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)	":59:849:80:50"
...
static ssize_t mecha_synaptics_virtual_keys_show_china_mfg(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)	":65:849:80:50"
...
static ssize_t mecha_synaptics_virtual_keys_show_china(struct kobject *kobj,
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf,
		__stringify(EV_KEY) ":" __stringify(KEY_BACK)	":65:849:80:50"
...
static struct kobj_attribute mecha_himax_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.himax-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &mecha_himax_virtual_keys_show,
};

static struct kobj_attribute mecha_atmel_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.atmel-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &mecha_atmel_virtual_keys_show,
};

static struct kobj_attribute mecha_synaptics_virtual_keys_attr = {
	.attr = {
		.name = "virtualkeys.synaptics-rmi-touchscreen",
		.mode = S_IRUGO,
	},
	.show = &mecha_synaptics_virtual_keys_show,
};

static struct attribute *mecha_properties_attrs[] = {
	&mecha_atmel_virtual_keys_attr.attr,
	&mecha_synaptics_virtual_keys_attr.attr,
	&mecha_himax_virtual_keys_attr.attr,
	NULL
};
...
static struct cm3629_platform_data cm36282_pdata = {
	...
	.intr = PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_PS_INT_N),
...
static struct i2c_board_info i2c_devices[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(MECHA_GPIO_TP_ATT_N)
	},
	{
		I2C_BOARD_INFO(SYNAPTICS_3200_NAME, 0x40 >> 1),
		.platform_data = &mecha_ts_synaptics_data,
		.irq = MSM_GPIO_TO_INT(MECHA_GPIO_TP_ATT_N)
	},
	{
...
static struct i2c_board_info i2c_Sensors_devices[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_GSENSOR_INT)),
	},
};
*/
...
*/
	struct pm8xxx_gpio_init_info gpio15 = {
		PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_FLASH_EN),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 0,
	...
	struct pm8xxx_gpio_init_info gpio19 = {
		PM8058_GPIO_PM_TO_SYS(MECHA_AUD_AMP_EN),
		{
			.direction      = PM_GPIO_DIR_OUT,
			.output_buffer  = PM_GPIO_OUT_BUF_CMOS,
			.output_value   = 0,
	...
	if (rc) {
		pr_err("%s MECHA_GPIO_FLASH_EN config failed\n", __func__);
		return rc;
	}
...
static struct tps65200_platform_data tps65200_data = {
	.gpio_chg_int = MSM_GPIO_TO_INT(PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_CHG_INT)),
};
...
#ifdef CONFIG_S5K4E5YX
static int mecha_sensor_version(void)
{
	if (sensor_version == 999) {
		if (gpio_get_value(MECHA_GPIO_CAM_ID) == 0) {
			sensor_version = MECHA_S5K4E5YX_EVT2;
		} else {
			sensor_version = MECHA_S5K4E5YX_EVT1;
		}
	}
...
static int Primou_s5k4e5yx_vreg_on(void)
{
	...
	if (mecha_sensor_version() == MECHA_S5K4E5YX_EVT2) {
		/* EVT2 V_CAM_D1V5 */
...
static struct msm_camera_sensor_info msm_camera_sensor_s5k4e5yx_data = {
	...
	.sensor_pwd = MECHA_GPIO_CAM1_PWD,
	.vcm_pwd = MECHA_GPIO_CAM1_VCM_PWD,
	...
	.use_rawchip = 1,
	.sensor_version = mecha_sensor_version,
...
static int mecha_use_ext_1v2(void)
{

		return 1;

}
static int mecha_rawchip_vreg_on(void)
{
	int rc;
	...
	if (system_rev >= 2) {
		/* V_RAW_1V2_EN*/
		rc = gpio_request(MECHA_GPIO_RAW_1V2_EN, "V_RAW_1V2_EN");
		if (rc) {
			pr_err("[CAM] sensor_power_enable\
				(\"gpio %d\", 1.8V) FAILED %d\n",
				MECHA_GPIO_RAW_1V2_EN, rc);
		} else {
			gpio_direction_output(MECHA_GPIO_RAW_1V2_EN, 1);
			gpio_free(MECHA_GPIO_RAW_1V2_EN);
		}		
	} else {
		/* V_RAWCSI_1V2 */
...
static int mecha_rawchip_vreg_off(void)
{
	int rc = 1;
	...
	if (system_rev >= 2) {
		...
		if (rc) {
			...
			gpio_direction_output(MECHA_GPIO_RAW_1V2_EN, 0);
			gpio_free(MECHA_GPIO_RAW_1V2_EN);
		}		
	} else {
		/* V_RAW_1V2 */
...
static uint32_t rawchip_on_gpio_table[] = {
	GPIO_CFG(MECHA_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP Reset */
	GPIO_CFG(MECHA_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT0 */
	GPIO_CFG(MECHA_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT1 */
	GPIO_CFG(MECHA_GPIO_MCAM_SPI_CS, 2 , GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
#if 0
	GPIO_CFG(MECHA_GPIO_CAM1_VCM_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(MECHA_GPIO_CAM1_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
#endif
	GPIO_CFG(MECHA_GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* MCLK */
	GPIO_CFG(MECHA_GPIO_CAM_I2C_SCL, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* I2C SCL*/
	GPIO_CFG(MECHA_GPIO_CAM_I2C_SDA, 2, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_8MA), /* I2C SDA*/
	GPIO_CFG(MECHA_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP Reset */
	GPIO_CFG(MECHA_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT0 */
	GPIO_CFG(MECHA_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP INT1 */
};
...
static uint32_t rawchip_off_gpio_table[] = {
	GPIO_CFG(MECHA_GPIO_CAM1_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(MECHA_GPIO_CAM1_VCM_PWD, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(MECHA_GPIO_CAM_MCLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), /* MCLK */
	GPIO_CFG(MECHA_GPIO_CAM_I2C_SCL, 2, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA), /* I2C SCL*/
	GPIO_CFG(MECHA_GPIO_CAM_I2C_SDA, 2, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_8MA), /* I2C SDA*/
	GPIO_CFG(MECHA_GPIO_CAM_MCLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_16MA), /* MCLK */
	GPIO_CFG(MECHA_GPIO_RAW_RSTN, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), /* RAW CHIP Reset */
	GPIO_CFG(MECHA_GPIO_RAW_INTR0, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* RAW CHIP INT0 */
	GPIO_CFG(MECHA_GPIO_RAW_INTR1, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), /* RAW CHIP INT1 */
	GPIO_CFG(MECHA_GPIO_MCAM_SPI_CS, 0 , GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA),
};
...
static struct msm_camera_rawchip_info msm_rawchip_board_info = {
	.rawchip_reset	= MECHA_GPIO_RAW_RSTN,
	.rawchip_intr0	= MECHA_GPIO_RAW_INTR0,
	.rawchip_intr1	= MECHA_GPIO_RAW_INTR1,
	...
	.camera_rawchip_power_on = mecha_rawchip_vreg_on,
	.camera_rawchip_power_off = mecha_rawchip_vreg_off,
	.rawchip_gpio_on = config_rawchip_on_gpios,
	.rawchip_gpio_off = config_rawchip_off_gpios,
	.rawchip_use_ext_1v2 = mecha_use_ext_1v2,
};
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.nluns = 2,
	.usb_id_pin_gpio = MECHA_GPIO_USB_ID1_PIN,
};
...
static struct i2c_board_info i2c_bma250_devices[] = {
	{
		...
		.irq = MSM_GPIO_TO_INT(MECHA_GPIO_GSENSOR_INT),
	},
};
...
static uint32_t msm_spi_on_gpio[] = {
       GPIO_CFG(MECHA_GPIO_MCAM_SPI_DO, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
       GPIO_CFG(MECHA_GPIO_MCAM_SPI_DI, 1, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_4MA),
       GPIO_CFG(MECHA_GPIO_MCAM_SPI_CS, 2 /*3*/, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
       GPIO_CFG(MECHA_GPIO_MCAM_SPI_CLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
#endif
...
static uint32_t qsd_spi_gpio_on_table[] = {
	PCOM_GPIO_CFG(MECHA_GPIO_MCAM_SPI_CLK, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_CFG_4MA),
	PCOM_GPIO_CFG(MECHA_GPIO_MCAM_SPI_DO, 1, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_CFG_4MA),
	PCOM_GPIO_CFG(MECHA_GPIO_MCAM_SPI_DI, 1, GPIO_INPUT, GPIO_PULL_DOWN, GPIO_CFG_4MA),
	PCOM_GPIO_CFG(MECHA_GPIO_MCAM_SPI_CS, 2, GPIO_OUTPUT, GPIO_NO_PULL, GPIO_CFG_4MA)
};
...
static struct msm_serial_hs_platform_data msm_uart_dm1_pdata = {
	...
	.bt_wakeup_pin = MECHA_GPIO_BT_WAKE,
...
#define HEADSET_DETECT		MECHA_GPIO_35MM_HEADSET_DET
#define HEADSET_BUTTON		MECHA_AUD_REMO_PRES
...
static struct TPS61310_flashlight_platform_data mecha_flashlight_data = {
	.gpio_init = config_flashlight_gpios_tps61310,
	.tps61310_strb1 = MECHA_GPIO_TORCH_EN,
	.tps61310_strb0 = PM8058_GPIO_PM_TO_SYS(MECHA_GPIO_FLASH_EN),
	.flash_duration_ms = 600,
	.led_count = 1,
};

static struct i2c_board_info mecha_flashlight[] = {
{
...
static uint32_t usb_ID_PIN_input_table[] = {
	GPIO_CFG(MECHA_GPIO_USB_ID1_PIN, 0, GPIO_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};

static uint32_t usb_ID_PIN_ouput_table[] = {
	GPIO_CFG(MECHA_GPIO_USB_ID1_PIN, 0, GPIO_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_4MA),
};
...
void config_mecha_usb_id_gpios(bool output)
{
	if (output) {
		...
		gpio_set_value(MECHA_GPIO_USB_ID1_PIN, 1);
		printk(KERN_INFO "%s %d output high\n",  __func__, MECHA_GPIO_USB_ID1_PIN);
	} else {
...
#endif
#ifdef CONFIG_FLASHLIGHT_TPS61310
	i2c_register_board_info(0, mecha_flashlight,
	ARRAY_SIZE(mecha_flashlight));
...
#endif
	if (board_mfg_mode() == 1) {
		for (i = 0; i < ARRAY_SIZE(mecha_ts_synaptics_data);  i++)
			mecha_ts_synaptics_data[i].mfg_flag = 1;
	}
...
*/
	...
	if (strstr(device_mid, "PK7612")) {
		if (1 == board_mfg_mode()) {
			mecha_synaptics_virtual_keys_attr.show = &mecha_synaptics_virtual_keys_show_china_mfg;
			mecha_himax_virtual_keys_attr.show = &mecha_himax_virtual_keys_show_china_mfg;
		} else {
			mecha_synaptics_virtual_keys_attr.show = &mecha_synaptics_virtual_keys_show_china;
			mecha_himax_virtual_keys_attr.show = &mecha_himax_virtual_keys_show_china;
		}
	}
