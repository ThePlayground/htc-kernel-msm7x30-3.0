...
#define PMGPIO(x) (x-1)
#define MECHA_ACDB_SMEM_SIZE        (0xE000)
#define MECHA_ACDB_RADIO_BUFFER_SIZE (1024 * 3072)
...
static unsigned aux_pcm_gpio_off[] = {
	GPIO_CFG(MECHA_GPIO_BT_PCM_OUT, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),	/* PCM_DOUT */
	GPIO_CFG(MECHA_GPIO_BT_PCM_IN, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),	/* PCM_DIN  */
	GPIO_CFG(MECHA_GPIO_BT_PCM_SYNC, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),	/* PCM_SYNC */
	GPIO_CFG(MECHA_GPIO_BT_PCM_CLK, 0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),	/* PCM_CLK  */
};


static unsigned aux_pcm_gpio_on[] = {
	GPIO_CFG(MECHA_GPIO_BT_PCM_OUT, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),	/* PCM_DOUT */
	GPIO_CFG(MECHA_GPIO_BT_PCM_IN, 1, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),	/* PCM_DIN  */
	GPIO_CFG(MECHA_GPIO_BT_PCM_SYNC, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),	/* PCM_SYNC */
	GPIO_CFG(MECHA_GPIO_BT_PCM_CLK, 1, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),	/* PCM_CLK  */
};
...
void mecha_hs_n1v8_enable(int en)
{
	int rc = 0;
...
void mecha_snddev_poweramp_on(int en)
{
	...
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_SPK_EN), 1);
		mdelay(30);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_SPEAKER;
		mdelay(5);
	} else {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_SPK_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_SPEAKER;
	}
}
...
void mecha_snddev_hsed_pamp_on(int en)
{
	...
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_HP_EN), 1);
		mdelay(30);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_HEADSET;
		mdelay(5);
	} else {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_HP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_HEADSET;
	}
}
/*
void mecha_snddev_pre_hsed_pamp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);

	if (en) {
		mecha_hs_n1v8_enable(1);
	} else {
		mecha_hs_n1v8_enable(0);
	}
}
*/
...
void mecha_snddev_receiver_pamp_on(int en)
{
	pr_aud_info("%s %d\n", __func__, en);
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_EP_EN), 1);
		mdelay(5);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode |= BIT_RECEIVER;
	} else {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_EP_EN), 0);
		if (!atomic_read(&aic3254_ctl))
			curr_rx_mode &= ~BIT_RECEIVER;
	}
}
...
void mecha_snddev_emic_pamp_on(int en)
{
	...
	if (en) {
		...
		if (system_rev > 0)
			gpio_set_value(
				PM8058_GPIO_PM_TO_SYS(MECHA_AUD_MICPATH_SEL_XB)
				, 1);
		else
			gpio_set_value(MECHA_AUD_MICPATH_SEL_XA, 1);
	} else {
		if (system_rev > 0)
			gpio_set_value(
				PM8058_GPIO_PM_TO_SYS(MECHA_AUD_MICPATH_SEL_XB)
			...
			gpio_set_value(MECHA_AUD_MICPATH_SEL_XA, 0);
	}
}

void mecha_snddev_fmspk_pamp_on(int en)
{
	if (en) {
		msleep(30);
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_SPK_EN), 1);
...
void mecha_snddev_fmhs_pamp_on(int en)
{
	if (en) {
		gpio_set_value(PM8058_GPIO_PM_TO_SYS(MECHA_AUD_HP_EN), 1);
		if (!atomic_read(&aic3254_ctl))
...
void mecha_rx_amp_enable(int en)
{
	if (curr_rx_mode != 0) {
		...
		if (curr_rx_mode & BIT_FM_SPK)
			mecha_snddev_fmspk_pamp_on(en);
		if (curr_rx_mode & BIT_FM_HS)
			mecha_snddev_fmhs_pamp_on(en);
		atomic_set(&aic3254_ctl, 0);
	}
}

uint32_t mecha_get_smem_size(void)
{
	return MECHA_ACDB_SMEM_SIZE;
}
...
//	.qtr_headset_enable = mecha_snddev_pre_hsed_pamp_on
};

static struct q5v2audio_icodec_ops iops = {
	.support_aic3254 = mecha_support_aic3254,
//	.support_adie = mecha_support_adie,
};
...
//	.support_beats = mecha_support_beats,
	.get_acoustic_tables = mecha_get_acoustic_tables
//	.enable_beats = mecha_enable_beats
};
...
static struct aic3254_ctl_ops cops = {
	...
	.panel_sleep_in = mecha_panel_sleep_in
};
...
void __init mecha_audio_init(void)
{
	...
	pm8xxx_gpio_config(MECHA_AUD_HP_EN, &audio_pwr_28);
	pm8xxx_gpio_config(MECHA_AUD_EP_EN, &audio_pwr_28);
	pm8xxx_gpio_config(MECHA_AUD_SPK_EN, &audio_pwr_28);
	pm8xxx_gpio_config(MECHA_AUD_MICPATH_SEL_XB, &audio_pwr_28);

	pm8xxx_gpio_config(MECHA_AUD_A3254_RSTz, &audio_pwr_18);
	mdelay(1);
