...
#define MECHA_SDMC_CD_N_SYS	PM8058_GPIO_PM_TO_SYS(MECHA_SDMC_CD_N)
...
static unsigned int mecha_sdslot_status(struct device *dev)
{
	...
	status = (unsigned int) gpio_get_value(MECHA_SDMC_CD_N_SYS);
	return !status;
}
...
static struct mmc_platform_data mecha_sdslot_data = {
	...
	.status_irq	= MSM_GPIO_TO_INT(MECHA_SDMC_CD_N_SYS),
...
static int mecha_wifi_cd = 0;	/* WIFI virtual 'card detect' status */
...
static int mecha_wifi_status_register(void (*callback)(int card_present,
							  void *dev_id),
					 void *dev_id)
{
	if (wifi_status_cb)
...
int mecha_wifi_power_state=0;
int mecha_bt_power_state=0;
...
int mecha_wifi_bt_sleep_clk_ctl(int on, int id)
{
	...
	if (on) {
		if ((CLK_OFF == mecha_sleep_clk_state_wifi)
			...
			err = pm8xxx_gpio_config(MECHA_WIFI_SLOW_CLK,
...
int mecha_bt_fastclock_power(int on)
{
	int rc;

	printk(KERN_DEBUG "mecha_bt_fastclock_power on = %d\n", on);
	if (vreg_wifi_osc) {
		if (on) {
			...
			printk(KERN_DEBUG "BT vreg_enable vreg_mmc, rc=%d\n",
			       rc);
			if (rc) {
				printk("Error turn mecha_bt_fastclock_power rc=%d\n", rc);
				return rc;
			}
		} else {
			if (!mecha_wifi_power_state) {
	...
	mecha_bt_power_state = on;
	return 0;
}
EXPORT_SYMBOL(mecha_bt_fastclock_power);

static int mecha_wifi_reset_state;
void mecha_wifi_reset(int on)
{
	printk(KERN_INFO "%s: do nothing\n", __func__);
}
#ifndef CONFIG_WIFI_CONTROL_FUNC
EXPORT_SYMBOL(mecha_wifi_reset);
#endif
...
//	register_msm_irq_mask(INT_SDC2_1);
	...
	msm_add_sdcc(2, &mecha_movinand_data);

	/* initial WIFI_SHUTDOWN# */
	id = PCOM_GPIO_CFG(MECHA_GPIO_WIFI_SHUTDOWN_N, 0, GPIO_OUTPUT,
...
//	register_msm_irq_mask(INT_SDC4_1);
	...
	irq_set_irq_wake(MSM_GPIO_TO_INT(MECHA_SDMC_CD_N_SYS), 1);
...
static int mechammc_dbg_wifi_reset_set(void *data, u64 val)
{
	mecha_wifi_reset((int) val);
	return 0;
}

static int mechammc_dbg_wifi_reset_get(void *data, u64 *val)
{
	*val = mecha_wifi_reset_state;
	return 0;
}

static int mechammc_dbg_wifi_cd_set(void *data, u64 val)
{
	mecha_wifi_set_carddetect((int) val);
	return 0;
}

static int mechammc_dbg_wifi_cd_get(void *data, u64 *val)
{
	*val = mecha_wifi_cd;
	return 0;
}

static int mechammc_dbg_wifi_pwr_set(void *data, u64 val)
{
	mecha_wifi_power((int) val);
	return 0;
}

static int mechammc_dbg_wifi_pwr_get(void *data, u64 *val)
{

	*val = mecha_wifi_power_state;
	return 0;
}

static int mechammc_dbg_sd_pwr_set(void *data, u64 val)
{
	mecha_sdslot_switchvdd(NULL, (unsigned int) val);
	return 0;
}

static int mechammc_dbg_sd_pwr_get(void *data, u64 *val)
{
	*val = sdslot_vdd;
	return 0;
}

static int mechammc_dbg_sd_cd_set(void *data, u64 val)
{
	return -ENOSYS;
}

static int mechammc_dbg_sd_cd_get(void *data, u64 *val)
{
	*val = mecha_sdslot_status(NULL);
	return 0;
}

DEFINE_SIMPLE_ATTRIBUTE(mechammc_dbg_wifi_reset_fops,
			mechammc_dbg_wifi_reset_get,
			mechammc_dbg_wifi_reset_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(mechammc_dbg_wifi_cd_fops,
			mechammc_dbg_wifi_cd_get,
			mechammc_dbg_wifi_cd_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(mechammc_dbg_wifi_pwr_fops,
			mechammc_dbg_wifi_pwr_get,
			mechammc_dbg_wifi_pwr_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(mechammc_dbg_sd_pwr_fops,
			mechammc_dbg_sd_pwr_get,
			mechammc_dbg_sd_pwr_set, "%llu\n");

DEFINE_SIMPLE_ATTRIBUTE(mechammc_dbg_sd_cd_fops,
			mechammc_dbg_sd_cd_get,
			mechammc_dbg_sd_cd_set, "%llu\n");

static int __init mechammc_dbg_init(void)
{
	struct dentry *dent;

	dent = debugfs_create_dir("mechammc_dbg", 0);
	if (IS_ERR(dent))
		return PTR_ERR(dent);

	debugfs_create_file("wifi_reset", 0644, dent, NULL,
			    &mechammc_dbg_wifi_reset_fops);
	debugfs_create_file("wifi_cd", 0644, dent, NULL,
			    &mechammc_dbg_wifi_cd_fops);
	debugfs_create_file("wifi_pwr", 0644, dent, NULL,
			    &mechammc_dbg_wifi_pwr_fops);

	debugfs_create_file("sd_pwr", 0644, dent, NULL,
			    &mechammc_dbg_sd_pwr_fops);
	debugfs_create_file("sd_cd", 0644, dent, NULL,
			    &mechammc_dbg_sd_cd_fops);

	return 0;
}

device_initcall(mechammc_dbg_init);
